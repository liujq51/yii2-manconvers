...
Return of `rbox\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `rbox\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `rbox\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
```php
<?php
user rbox\admin\components\Helper;
