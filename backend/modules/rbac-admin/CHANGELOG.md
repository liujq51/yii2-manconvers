...
- Enh: Custom side menu via `rbox\admin\Module::items` (mdmunir).
...
- Chg: Can customize return of `rbox\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `rbox\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `rbox\admin\Module::allowActions`. Set access control directly with `rbox\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `rbox\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
